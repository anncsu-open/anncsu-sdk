...
def get_security_from_env(security: Any, security_class: Any) -> Optional[BaseModel]:
    """Get security configuration from environment variables if not provided.

    This function checks if security is already provided. If not, it attempts
    to construct security credentials from environment variables.

    Args:
        security: Existing security configuration (may be None)
        security_class: The security model class to instantiate

    Returns:
        Security configuration from provided value or environment variables,
    ...
    if os.getenv("ANNCSU_BEARER_AUTH"):
        security_dict["bearer_auth"] = os.getenv("ANNCSU_BEARER_AUTH")

    if os.getenv("ANNCSU_AGID_JWT_SIGNATURE"):
        security_dict["agid_jwt_signature"] = os.getenv("ANNCSU_AGID_JWT_SIGNATURE")

    if os.getenv("ANNCSU_AGID_JWT_TRACKING_EVIDENCE"):
        security_dict["agid_jwt_tracking_evidence"] = os.getenv(
            "ANNCSU_AGID_JWT_TRACKING_EVIDENCE"
        )
