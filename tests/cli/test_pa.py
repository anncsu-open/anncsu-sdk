...
class TestAccesso:
    ...
    def test_accesso_enriches_denomuff_from_prognazarea(
        self, cli_runner: CliRunner
    ) -> None:
        """Test accesso fetches denomuff via prognazarea when missing."""
        from anncsu.cli import app

        # Create accesso response with denomuff=None (like real API)
        item = MagicMock()
        item.prognaz = "1222543"
        item.dug = "LARGO"
        item.denomuff = None
        item.denomloc = ""
        item.denomlingua1 = ""
        ...
        accesso_resp = MagicMock()
        accesso_resp.data = [item]

        # Create prognazarea response with denomuff
        area_item = MagicMock()
        area_item.denomuff = "CHIAFFREDO BERGIA"
        area_item.denomloc = ""
        area_item.denomlingua1 = ""
        area_item.denomlingua2 = ""
        area_resp = MagicMock()
        area_resp.data = [area_item]

        mock_sdk = MagicMock()
        mock_sdk.queryparam.prognazacc_get_query_param.return_value = accesso_resp
        mock_sdk.queryparam.prognazarea_get_query_param.return_value = area_resp

        with patch("anncsu.cli.commands.pa._get_consult_sdk", return_value=mock_sdk):
            result = cli_runner.invoke(
                app, ["pa", "accesso", "--prognazacc", "28586543", "--enrich"]
            )
    ...
    def test_accesso_enrichment_failure_graceful(self, cli_runner: CliRunner) -> None:
        """Test accesso continues gracefully if enrichment fails."""
        from anncsu.cli import app
        ...
        # Should still succeed, just without denomuff
        assert result.exit_code == 0
        assert "LARGO" in result.output

    def test_accesso_no_enrich_skips_prognazarea(self, cli_runner: CliRunner) -> None:
        """Test accesso without --enrich does not call prognazarea."""
        from anncsu.cli import app
        ...
        assert result.exit_code == 0
        mock_sdk.queryparam.prognazarea_get_query_param.assert_not_called()
